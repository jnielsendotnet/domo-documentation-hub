...
| ------------------ | ---------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| KitchenSales       | Represents the amount of sales for the Kitchen Department ONLY.              | `SUM(CASE WHEN `Department` = ‘Kitchen’ THEN `Sales` ELSE 0 END)`                                                                                      |
| NonKitchenSales    | Represents the amount of sales for all NON Kitchen Departments of the store. | `SUM(CASE WHEN `Department` = ‘Kitchen’ THEN 0 ELSE `Sales` END)`                                                                                      |
| ShareOfDepartments | Represents the percent of the all Sales for each individual Department.      | `(SUM(Sales) / SUM(Sales) OVER ()) * 100`<br/>This means it shows the sum of all departments.<br/>Multiply by 100 to show the value in percent format. |
