...
| Host/Domain       | Enter the host or domain name where your JSON files are located. <br/> For example: **[files.myhost.com](http://files.myhost.com)** or IP (**121.123.123.123** ). |
...
| Menu                                       | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| ------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| URL to Your JSON File                      | Enter the URL for the JSON file. Supported protocols are HTTPS and SFTP.<br/><Note>
**Note:** Due to security concerns, Domo no longer permits uploading data using FTP. If necessary, please update your server settings to support SFTP instead of FTP.
</Note>                                                                                                                                                                                                                                                                                           |
| Connection Method                          | Select whether you want to connect to your JSON file via file transfer (SFTP) or HTTP request.<br/>Select File Transfer (SFTP) if your JSON file is on an SFTP server.<br/>Select HTTP Request if you need to access your JSON file using an HTTP GET request. The request is triggered by the use of a unique URL parameter defined by the service providing the JSON file.                                                                                                                                                                                |
...
| Header Tag (Optional)                      | Enter the tag for the header in your JSON text. JSON headers are not normally included in JSON, but when they are included, they will most likely use one of the following formats:<br/>```json
{
 "People": {
 "name": "John",
 "age": 30,
...
```<br/>Note that the first example does not include the "header" tag, while the second does. In the second example, the headers are listed separately from the actual data tags. |
