...
| Field               | Description                                                                                                                                                                                                         |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Neo4j Instance URL  | Enter the URL of your instance.<br/>Example: neo4j.database.url or [graph.site.com](http://graph.site.com):7473.<br/><Note>**Note**: Include port number if applicable. The default port is 7473 for Neo4j. </Note> |
...
| Menu  | Description                                                                                                                                                                                                                                                                                                                                     |
| ----- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Query | Enter your Cypher query.<br/>**Example**:<br/>MATCH (p:Person)-[:BUYS]->(i:Item)<br/>RETURN [p.name](http://p.name) as Name,i.type as ItemType<br/>MATCH (a:Actor)-[:PRODUCES]->(m:Movie)<-[:DIRECTS]-(d:Director)<br/>RETURN [a.name](http://a.name) as ActorName, a.DOB as DOB,m.title as MovieTitle, [d.name](http://d.name) as DirectorName |
