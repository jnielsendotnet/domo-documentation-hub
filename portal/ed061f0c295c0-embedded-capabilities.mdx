...
sidebarTitle: 'Embed Edit Experiences'
---
...
- Timezone – For example: America/Denver or Asia/Tokyo. For a full list of valid timezone settings, see this article: [timezones](https://en.wikipedia.org/wiki/List_of_tz_database_time_zones)
- Group – A list of XML escaped strings

The payload must be signed using the provided secret and be encoded using an HS algorithm (we encourage HS256). A great site for learning more about JWT’s, creating them for testing purposes and finding access to different code repositories for creating and signing JWT’s is [jwt.io](http://jwt.io).
...
For more information also see the [Domo Knowledge Base](https://domo-support.domo.com/s/topic/0TO5w000000ZanbGAC/domo-everywhere?language=en_US). In particular, this article on [Routing, Creation, and Mapping](/s/article/6523741250455?language=en_US) may be helpful.
