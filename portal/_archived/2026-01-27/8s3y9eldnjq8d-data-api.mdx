...
<Card
  ...
  title="Playground"
  icon="code"
  href="/api-reference/data-api/query-a-dataset-data-api"
/>
...
Perform a `groupby` with predefined date grains. By default, numerical columns are summed and unique entries of non-numerical columns are counted. Can be combined with an [aggregation](/portal/8s3y9eldnjq8d-data-api#aggregations) to override this default behavior.
...
Aggregates identical data into groups. By default, numerical columns are summed and unique entries of non-numerical columns are counted. Can be combined with an [aggregation](/portal/8s3y9eldnjq8d-data-api#aggregations) to override this default behavior. Multiple columns can be grouped by delimiting each with a comma.
...
You can control the format of the data returned from this endpoint by setting the request `Accept` header. If leveraging the [domo.js](/portal/e947d87e17547-domo-js) library, the same can be accomplished by passing a `format` option in the request. Please refer to the [domo.get()](/portal/e947d87e17547-domo-js#domo-get) documentation for further details.
array-of-objects
...
```js
domo
  .get('/data/v1/dataAlias?useBeastMode=true&fields=beastModeAlias,reps&sum=beastModeAlias&groupby=reps',
  )
  .then(function (data) {
    console.log('data', data);
  });
...
```js
domo
  .get('/data/v1/dataAlias?useBeastMode=true&fields=beastModeAlias,reps&groupby=reps',
  )
  .then(function (data) {
    console.log('data', data);
  });
...
<Card
  ...
  title="Playground"
  icon="code"
  href="/api-reference/sql-api/query-a-dataset-sql-api"
/>
