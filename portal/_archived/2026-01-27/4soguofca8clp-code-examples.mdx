...
<?php
    ...
    if ($request == "/card") {
        ...
        curl_setopt_array($curl, array(CURLOPT_URL => "https://api.domo.com/oauth/token?grant_type=client_credentials&scope=data%20user%20dashboard",
        ...
        if ($err) {
            ...
            $curl2 = curl_init();
            curl_setopt_array($curl2, array(CURLOPT_URL => "https://api.domo.com/v1/cards/embed/auth",
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => "",
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 30,
            ...
            CURLOPT_HTTPHEADER => array("Authorization: Bearer ".$responsearray['access_token'],
