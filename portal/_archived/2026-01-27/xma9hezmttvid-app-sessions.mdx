...
Another option is to use the `domo.js` library, which can be included in the app (generally by loading it from the public-assets folder). Requests which have been sent—either as static asset requests or as functions, as supplied in `domo.js` (such as [`domo.get()`](/portal/e947d87e17547-domo-js#domoget))—will be modified to include either the query parameter (on static asset requests) or the header (on function calls). See the [`domo.js` documentation here](/portal/e947d87e17547-domo-js) for more details.
