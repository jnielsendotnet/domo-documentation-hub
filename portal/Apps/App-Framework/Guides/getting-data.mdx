---
title: Getting Data
permalink: 6a5d7a2870145-getting-data
---
...
Learn more in the manifest [reference](/portal/Apps/App-Framework/Guides/manifest) documentation.
...
where `[DataSet Alias]` is replaced with the desired DataSet alias from your design's [manifest.json file](/portal/Apps/App-Framework/Guides/manifest).
...
To get compute the sum of the `amount` column in the `sales` DataSet from the above sample mapping you could make a request with the [domo.js library](/portal/Apps/App-Framework/Tools/domo.js) in the following way:

```js
domo
  .post('/sql/v1/sales', 'SELECT SUM(amount) FROM sales', {
...
#### [domo.js](/portal/Apps/App-Framework/Tools/domo.js) (recommended)
...
```javascript
var salesSum = 0;
oboe('/data/v1/sales').node('!.*', function (obj) {
  salesSum += obj.amount;
});
```
...
```javascript
$http.get('/data/v1/sales').success(function (data, status, headers, config) {
  for (var i = 0; i < data.length; i++) $scope.salesSum += data[i].amount;
});
```
