...
`da new` prompts for a package manager, clones the template, writes your app name into `package.json`, initializes git, and installs dependencies.
...
```bash
yarn add @reduxjs/toolkit react-redux @domoinc/toolkit
```
...
This runs `yarn build` and `domo publish` from the `build/` folder. The output prints a link to the new App Design.
...
Each component lives in its own folder with a matching `.module.scss`. We'll show the TypeScript; use whatever styling you prefer — the [GitHub source](https://github.com/DomoApps/basic-react-app-todo-tutorial) has the SCSS if you want to copy it verbatim.
...
The avatar URL can 404 for users without an uploaded photo — the `onError` handler flips to a gradient fallback with the user's initials.
...
Ownership is stamped at submit time — the form reads `userInfo` from the store and attaches `ownerId` / `ownerName` to every new todo.
...
The root component wires everything together: kick off the two initial loads, surface any errors in a banner, and render the header + form + list.
...
The script runs `yarn build` (which lints, tests, and builds), then `domo publish` from the `build/` folder. The new version becomes the active design for anyone instantiating the app from your Asset Library.
