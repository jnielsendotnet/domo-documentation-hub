---
title: 'AI Book Recommender App Tutorial'
permalink: 'c07d31c752348-ai-book-recommender-app-tutorial'
sidebarTitle: 'AI Book Recommender App'
---
...
<Frame>
  ![screen-one.png](/images/dev/stoplight.io/api/v1/projects/cHJqOjI2NDA4OA/branches/YnI6MTE1MjI3Mjk/images/assets/tutorials/book-recommender/screen-one.png)
</Frame>
...
```html
<div className="form">
  <Select mode="multiple" autoClearSearchValue filterOption={false} allowClear
  placeholder="Choose your favorite books" options={[{value: 'test', label:
  'test'}]} style={{ flex: 1 }} />
</div>
<div className="form">
  <Select placeholder="Select a genre" options={[{value: 'test', label:
  'test'}]} style={{ flex: 1 }} /> <Select placeholder="Select a mood"
  options={[{value: 'test', label: 'test'}]} style={{ flex: 1 }} /> <Select
  placeholder="Select a book length" options={[{value: 'test', label: 'test'}]}
  style={{ flex: 1 }} />
</div>
```
...
```html
<div className="form">
  <Select placeholder="Select a genre" value={genre} onChange={(value) =>
  setGenre(value)} options={genres} style={{ flex: 1 }} /> <Select
  placeholder="Select a mood" value={mood} onChange={(value) => setMood(value)}
  options={moods} style={{ flex: 1 }} /> <Select placeholder="Select a book
  length" value={bookLength} onChange={(value) => setBookLength(value)}
  options={bookLengths} style={{ flex: 1 }} />
</div>
```
...
<Frame>
  ![screen-two.png](/images/dev/stoplight.io/api/v1/projects/cHJqOjI2NDA4OA/branches/YnI6MTE1MjI3Mjk/images/assets/tutorials/book-recommender/screen-two.png)
</Frame>
...
const onBookSearch = async (value) => {
  ...
  debounceTimer.current = setTimeout(async () => {
    ...
    setAllBooks((prevBooks) => [
      ...prevBooks,
      ...fetchedBooks.filter((newBook) =>!prevBooks.some((book) => book.key === newBook.key),
      ),
    ]);
...
const onSubmit = async () => {
  setLoading(true);
  const recs = await getBookRecommendations(favoriteBooks,
    genre,
    mood,
    bookLength,
  );
    ...
    >
      <div className="content">
        {recommendations.length === 0? (<>
            ...
            <h1>Recommended Books</h1>
            <div className="bookList">
              {recommendations.map((rec, index) => (<div key={index} className="bookItem">
                  <h4>{rec.title}</h4>
                  <p className="author">{rec.author}</p>
                  <p className="reason">{rec.reason}</p>
                </div>
...
<Frame>
  ![screen-three.png](/images/dev/stoplight.io/api/v1/projects/cHJqOjI2NDA4OA/branches/YnI6MTE1MjI3Mjk/images/assets/tutorials/book-recommender/screen-three.png)
</Frame>

<Frame>
  ![screen-four.png](/images/dev/stoplight.io/api/v1/projects/cHJqOjI2NDA4OA/branches/YnI6MTE1MjI3Mjk/images/assets/tutorials/book-recommender/screen-four.png)
</Frame>
