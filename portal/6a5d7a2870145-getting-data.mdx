...
Learn more in the manifest [reference](/portal/af407395c766b-the-manifest-file) documentation.
...
where `[DataSet Alias]` is replaced with the desired DataSet alias from your design's [manifest.json file](/portal/af407395c766b-the-manifest-file).
...
To get compute the sum of the `amount` column in the `sales` DataSet from the above sample mapping you could make a request with the [domo.js library](https://developer.domo.com/portal/domo.js.md) in the following way:
...
#### [domo.js (recommended)](#domojs-recommended)
